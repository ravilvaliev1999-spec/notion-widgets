...
var WidgetV19Core = (function () {
  ...
  function isGoogleDriveUrl(value) {
    return Boolean(googleHost(value));
  }
  ...
  function classify(input) {
    ...
    if (mime === GOOGLE_MIME.Docs || (!mime && host === 'docs.google.com' && /\/document\//.test(url))) {
      return { section: 'Docs', format: 'Google Docs', provider: 'Google Drive', knowledgeFormat: 'Файл' };
    }
    if (mime === GOOGLE_MIME.Sheets || (!mime && host === 'docs.google.com' && /\/spreadsheets\//.test(url))) {
      return { section: 'Sheets', format: 'Google Sheets', provider: 'Google Drive', knowledgeFormat: 'Файл' };
    }
    if (mime === GOOGLE_MIME.Slides || (!mime && host === 'docs.google.com' && /\/presentation\//.test(url))) {
  ...
  function describeGoogleMetadata(file, fallbackUrl) {
    file = file || {};
    var id = String(file.id || '').trim();
    var mimeType = String(file.mimeType || '').trim();
    var name = cleanName(file.name, 'Без названия');
    var candidateUrl = String(file.webViewLink || fallbackUrl || '').trim();
    var detected = classify({ name: name, mimeType: mimeType, url: candidateUrl });
    var sourceUrl = candidateUrl;
    if (!sourceUrl && mimeType === 'application/vnd.google-apps.folder' && id) {
      sourceUrl = 'https://drive.google.com/drive/folders/' + encodeURIComponent(id);
    }
    if (!sourceUrl && id) sourceUrl = makeDriveOpenUrl(id, detected.format);
    return {
      id: id,
      name: name,
      mimeType: mimeType,
      ...
      sourceUrl: sourceUrl,
      normalizedUrl: normalizeExternalUrl(sourceUrl || fallbackUrl) || '',
      size: file.size === null || file.size === undefined || file.size === '' ? null : Number(file.size),
      driveMd5: String(file.md5Checksum || '')
    };
  }
  ...
  return Object.freeze({
    ...
    describeGoogleMetadata: describeGoogleMetadata,
    safeEqual: safeEqual
  });
}());
